...
equations=function(t,state,parameters) 	# function containing the diferential equations
	{with(as.list(c(state,parameters)),
		...
		V_patp = .03 * PMPA/(PMPA + .803)*(PMPA/0.01672982)*(DAG/0.006625219)
		V_source = 0.273
		V_cds = .521 * ERPA/(ERPA + .106)
		V_pis = 15.943 * CDPDAG/(CDPDAG + 0.045)
		V_pitp = 3.314* ERPI/(ERPI + 1.299)*(PMPA/0.01672982)*(DAG/0.006625219)
...
pval=c(NA,
	...
	parameters[names(parameters)=='dagk_Vmax']*.32,
...
# On the third the new value for the variable.
pval=c(NA,
	parameters[names(parameters)=='dagk_Vmax']*0.10,
	parameters[names(parameters)=='dagk_Vmax'],
	NA)
perturbations=data.frame(time=ptime,var=pvar,val=pval)
perturbations
...
text(x=c(1,2,3,4),y=RdgA3_vals-.1,labels =round(RdgA3_vals,2))
...
text(x=c(1,2,3,4),y=laza22_vals-.15,labels =round(laza22_vals,2))
legend(4,2.5,legend=c('WT','Mutant'),fill=c('darkgreen','red'))
