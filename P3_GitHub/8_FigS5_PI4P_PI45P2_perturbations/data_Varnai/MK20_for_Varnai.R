### Model ###
### This is basal - bistable ###

### Competition in the phosphatases ###
### regulation in PI3KI and PTEN    ###
### PLC, DAG, DGK, PA               ###
### PA and DAG 25% of PI4P		###
### 82% of PA comes from DGK		###
...
mp1_Ca = 1				# Calcium metaparameter
mp2_PI3KI = 0.0006217539	# metaparameter for PI3KI
mp3_PTEN = 0.003			# metaparameter for PTEN
mp4_PLC = .3			# PLC metaparameter

HS = function (PA)(1 + 1.7*PA^3/(PA^3+7000^3))

PS = 150000

# Inicial states for variables
# insert, in the parentesis, the inicial value of the dependent variables (ex: X=2) 
state=c(PI =  297877.9,PI3P = 96.80468, PI4P = 9835.422, PI5P = 98.36936,
	PI35P2 = 21.57369, PI45P2 = 10042.65, PI34P2 = 44.92887,
	PI345P3 = 457.4147,
	pi_3KI_a = 281.6386, PTEN_a = 2.008187,
	DAG=6681.578,
	IP3=1386.709,
	PA=4430.644,
	Ca = 60,
	PIP5KI_PI4P=70,
	pi_4K_pip_5KI_PI4P=20,
	PKC = 12,
	PLC = 5,
	GPCR = 8.786603
	) 

# parameters and variables that are doing nothing and could be deleted
# pi_3KIc
# PTENc
...
parameters=c(
		...
		pi_4K = 450,		   	# type II. Phosphorilate PI into PI4P. Very similar to PI_3Ks in structure. Type I was after all, PI_3KII 
		pi_Kfyve = 48,			# type III PIP_5K. Really diferent from the other PIP_5K.
		pip_5KIt = 600,			# Phosphorilate PI4P into PI45P2. Activated by PA.
		pip_5KII = 845,    		# Phosphorilate PI5P into PI45P2 
		pi_4K_pip_5KIt = 250,		# this is here because PI45P2 should not be inflenced by PI4P depletion
		...
		PLCt = 10,  		# Phospholipase C. Breaks PI45P2 into DAG and IP3.
		DGK = 30,  			# Diacylglicerol Kinase. Transforms DAG into PA.
		...
		gamma45_4d = 5.137691e+13, f45_4d_PI45P2 = 9.378806e-01, f45_4d_PI35P2 = -3.105968e-04, f45_4d_PI345P3 = -2.363093e-03,
...
parameters=c(parameters,
		...
		gammaPTENc_PTENa = mp3_PTEN * 4.166667e-06, fPTENc_PTENa_PTEN_c = 1, fPTENc_PTENa_PI45P2 = 1, 
		...
		gamma_DAG = 0.00001,
		gammaDAG_ = .11, f104_DAG = 1,
		gamma_PA = 4,
		gammaPA_ = 0.11, f106_PA = 1,
		gammaPC_PA = 3, f_PC_PA_PKC = .63, f_PC_PA_PI45P2 = .3,
		gammaIP3_ = .5, f107_IP3 = 1,

		gamma_5K_5K4P = .001, f_5K_PA_inactiv = -.3,
		gamma_5K4P_5K = 0.17, f_5K4P_5K_PA = .2,

		gamma_4K5K_4K5K4P = .003, f_4K5K_PA_inactiv = -.3, 
		gamma_4K5K4P_4K5K =  0.1584893, f_4K5K4P_4K5K_PA = .2,

		Ca_t = 1000, 					# total Calcium
		f_IP3_Ca_IP3 = 8, f_IP3_Ca_CaER = 1
		)

parameters = c(parameters,
		gamma_Ca =  as.numeric(mp1_Ca * ( state[names(state)=='Ca']/( parameters[names(parameters)=='Ca_t'] - state[names(state)=='Ca'] ) )), f__Ca_CaER = 1, 
		gammaIP3_Ca =  0, 
		gammaCa_ = 1, f_Ca_Ca = 1,

		gamma_PKC_in = .2, f_PKC_in_CA = 1,
		gamma_PKC_out = 1, f_PKC_out_PKC = 1,

		f_PLC_i_a_PLCi = 1, f_PLC_i_a_Ca = .3, f_PLC_a_i_GPCR = .7
		)

parameters = c(parameters,
		gamma_PLC_i_a = as.numeric(mp4_PLC * (state[names(state)=='PLC']/((parameters[names(parameters)=='PLCt'] - state[names(state)=='PLC'])*(state[names(state)=='Ca']^parameters[names(parameters)=='f_PLC_i_a_Ca'])*(state[names(state)=='GPCR']^parameters[names(parameters)=='f_PLC_a_i_GPCR'])))), 
		gamma_PLC_a_i = mp4_PLC, f_PLC_a_i_PLC = 1,

		gamma_GPCR_in = .6, f_GPCR_in_PKC = -.331,
		gamma_GPCR_out = .03, f_GPCR_out_GPCR = 1,
...
equations=function(t,state,parameters) 	# function containing the diferential equations
	{with(as.list(c(state,parameters)),
		...
		pip_5KI = pip_5KIt - PIP5KI_PI4P

		pi_4K_pip_5KI = pi_4K_pip_5KIt - pi_4K_pip_5KI_PI4P
	
		PLCi = PLCt - PLC
		V_PLC_i_a = gamma_PLC_i_a * (PLCt - PLC)^f_PLC_i_a_PLCi * Ca^f_PLC_i_a_Ca * GPCR^f_PLC_a_i_GPCR
		V_PLC_a_i = gamma_PLC_a_i * PLC^f_PLC_a_i_PLC 
		...
		V45_0_inpp5e = gamma45_4d * PI45P2^f45_4d_PI45P2 * INPP5E^2 * 70.205 *1.66*10^-18 
		...
		V_DAG = gamma_DAG 
		VDAG_ = gammaDAG_ * DAG^f104_DAG 
		V_PA = gamma_PA 
		VPA_ = gammaPA_ * PA^f106_PA
		VPC_PA = gammaPC_PA * PKC^f_PC_PA_PKC * PI45P2^f_PC_PA_PI45P2
		VIP3_ = gammaIP3_ * IP3^f107_IP3

		V5K_5K4P = gamma_5K_5K4P * PI4P * pip_5KI * PA^f_5K_PA_inactiv
		V5K4P_5K = gamma_5K4P_5K * PIP5KI_PI4P * PA^f_5K4P_5K_PA

		V4K5K_4K5K4P = gamma_4K5K_4K5K4P * PI4P * pi_4K_pip_5KI * PA^f_4K5K_PA_inactiv
		V4K5K4P_4K5K = gamma_4K5K4P_4K5K * pi_4K_pip_5KI_PI4P * PA^f_4K5K4P_4K5K_PA
		
		V_Ca = mp1_Ca * ( state[names(state)=='Ca']/( Ca_t - state[names(state)=='Ca'] ) ) * (Ca_t - Ca)^f__Ca_CaER
		VIP3_Ca = gammaIP3_Ca * IP3^f_IP3_Ca_IP3 * (Ca_t - Ca)^f_IP3_Ca_CaER
		VCa_ = mp1_Ca * Ca^f_Ca_Ca

		V_PKC_in = gamma_PKC_in * Ca^f_PKC_in_CA
		V_PKC_out = gamma_PKC_out * PKC^f_PKC_out_PKC

		V_GPCR_in = gamma_GPCR_in * PKC^f_GPCR_in_PKC
		V_GPCR_out = gamma_GPCR_out * GPCR^f_GPCR_out_GPCR

		V_PI4P_DAG = gamma4_DAG * PI4P^(f4_DAG_PI4P) * PI45P2^(f4_DAG_PI45P2) * PS^(f4_DAG_PS) * PLC * 88.4215 *1.66*10^-18

		dPI = V_0 + V3_0a + V3_0c + V3_0d + V3_0e + V4_0a + V4_0c + V5_0a  + V5_0c + V45_0 - V0_3a - V0_3b - V0_3c - V0_4 - V0_5 - V0_ - V0_45 + V45_0_inpp5e
		...
		dPI45P2 = V4_45 + V5_45 + V345_45 + V0_45 - V45_4a - V45_4c - V45_4d - V45_5a  - V45_5c - V45_345 - V45_ - V45_0 - V45_DAG - V45_0_inpp5e
		...
		dDAG = V_DAG + VPA_DAG  + V_PI4P_DAG + V45_DAG - VDAG_PA - VDAG_ #+ V_PI_DAG
		dIP3 = V45_DAG - VIP3_
		dPA = V_PA + VDAG_PA + VPC_PA - VPA_DAG - VPA_ 
		dCa = 0 # V_Ca + VIP3_Ca - VCa_

		dPIP5KI_PI4P = 0 #V5K_5K4P - V5K4P_5K

		dpi_4K_pip_5KI_PI4P = 0 #V4K5K_4K5K4P - V4K5K4P_4K5K

		dPKC = 0 #V_PKC_in - V_PKC_out

		dPLC = 0 # V_PLC_i_a - V_PLC_a_i	

		dGPCR = V_GPCR_in - V_GPCR_out

		# return the rate of change 
		# insert, in the parentecis, the variables that you want to store the values (ex:dX)
		list(dy=c(dPI, dPI3P, dPI4P, dPI5P,dPI35P2,dPI45P2,dPI34P2,dPI345P3,dpi_3KI_a,dPTEN_a,dDAG,dIP3,dPA,dCa,dPIP5KI_PI4P,dpi_4K_pip_5KI_PI4P,dPKC,dPLC,dGPCR),
				count=c(pip_5KI,pi_4K_pip_5KI,
					...
					V0_45,V4_34a,V4_34b,V34_4,V45_0,V45_0_inpp5e,
					VPI3KIc_PI3KIa,VPI3KIa_PI3KIc,VPTENc_PTENa,VPTENa_PTENc,
					V45_DAG,VPA_DAG,VDAG_PA,V_DAG,VDAG_,V_PA,VPA_,VPC_PA,VIP3_,
					V5K_5K4P,V5K4P_5K,
					V4K5K_4K5K4P,V4K5K4P_4K5K,
					V_Ca, VIP3_Ca,VCa_,
					V_PKC_in,V_PKC_out,
					PLCi,V_PLC_i_a,V_PLC_a_i,
					V_GPCR_in,V_GPCR_out,
					V_PI4P_DAG
					))
		})
	}
...
#	state[18]*5,
#	state[18],
#	parameters[names(parameters)=='DGK']*0.10,
...
plot(out[,1],out[,16],ylim=c(0,200),type='l',col=varcolors[16],main='PIP5KI')
text(tmax,out[tmax-1,16],varnames[16],col=varcolors[16])
points(out[,1],out[,21],type='l',col='violet')
text(tmax,out[tmax-1,21],names(out[tmax-1,21]),col='violet')

plot(out[,1],out[,17],ylim=c(0,200),type='l',col=varcolors[17],main='PI4K_PIP5KI_DVL')
text(tmax,out[tmax-1,17],varnames[17],col=varcolors[17])
points(out[,1],out[,22],type='l',col='mediumvioletred')
text(tmax,out[tmax-1,22],names(out[tmax-1,22]),col='mediumvioletred')

plot(out[,1],out[,19],ylim=c(0,20),type='l',col=varcolors[19],main='PLC')
text(tmax,out[tmax-1,19],varnames[19],col=varcolors[19])
points(out[,1],out[,94],type='l',col='mediumvioletred')
text(tmax,out[tmax-1,94],names(out[tmax-1,94]),col='mediumvioletred')
par(mfrow=c(1,1))
...
points(out[,1],out[,15],type='l',col=varcolors[15])
text(tmax,out[tmax-1,15],varnames[15],col=varcolors[15])
points(out[,1],out[,18],type='l',col=varcolors[18])
text(tmax,out[tmax-1,18],varnames[18],col=varcolors[18])
points(out[,1],out[,20],type='l',col=varcolors[20])
text(tmax,out[tmax-1,20],varnames[20],col=varcolors[20])
