#################################################
#								#
#	Surdekar and ENaC ASL model 5 Po 1		#
#								#
#################################################
...
# x = seq(.0001,.3,.0001)
# K = 0.01672982
# Vmax_calc = (1.055 - .3) / ( 0.01672982^2 / ( K^2 + 0.01672982^2 ) )
# Hill = function (x,Vmax = Vmax_calc, K = 0.01672982 , h = 2)
#	{
#	.3 + Vmax* x^h / ( K^h + x^h )
#	}
# Hill(x=0.01672982)
# Hill(x=0.0001626)
# plot(x,Hill(x=x),ylim=c(0,3))
#########################################################################
...
equations=function(t,state,parameters) 	# function containing the diferential equations
	{with(as.list(c(state,parameters)),
		...
		V_pip5k = (.3 + pip5k_Vmax_0 * PMPA^2 / (0.01672982^2 + PMPA^2) )* PI4P/(PI4P + 0.109)
		...
		V_pi4p_exit =  0.01174093 * PI4P / (PI4P + 0.109)

		dPMPI = V_pitp - V_pi4k
		dPI4P = V_pi4k - V_pip5k - V_pi4p_exit
		dPIP2 = V_pip5k - V_plc + V_pi4p_exit
...
# On the third the new value for the variable.
pval=c(NA,
	parameters[names(parameters)=='dagk_Vmax']*0.20,
	parameters[names(parameters)=='dagk_Vmax'],
	NA)
perturbations=data.frame(time=ptime,var=pvar,val=pval)
perturbations
...
# tiff("P2F2_Fig_S9.tiff", height = 20, width = 34, units = 'cm',compression = "lzw", res = 300)
...
axis(1, at=bp,labels=as.character(name_perturb), las=1, cex.axis = 2)
par(mfrow=c(1,1))
...
barplot(
	laza22_vals,
	col=c('darkblue','red','darkblue','red'),
	main = 'Laza',cex.main=3,cex.names=2,
	cex.axis=2
		)
text(x=c(.7,1.95,3.1,4.3),y=laza22_vals-.20,labels =round(laza22_vals,2),col='white',cex=3)
legend(3,3,legend=c('WT','Mutant'),fill=c('darkblue','red'),bty='n',cex=3)
