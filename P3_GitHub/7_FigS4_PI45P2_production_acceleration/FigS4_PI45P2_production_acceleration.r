...
# for basolateral divide gammaPTENc_PTENa by 10
...
equations=function(t,state,parameters) 	# function containing the diferential equations
	{with(as.list(c(state,parameters)),
		...
		V0_45 = gamma0_45 * PI^f0_45_PI * pi_4K_pip_5KI * 248.608 * 1.66*10^-18  * HS(PA) * PI45P2^f0_45_PI45P2 * 20
		...
		dPI = V45_ + V_0 + V3_0a + V3_0c + V3_0d + V3_0e + V4_0a + V4_0c + V5_0a  + V5_0c + V45_0 - V0_3a - V0_3b - V0_3c - V0_4 - V0_5 - V0_ - V0_45 #- V_PI_DAG
...
#	parameters[names(parameters)=='PLC']*1.01,
#	parameters[names(parameters)=='PLC'],
#	parameters[names(parameters)=='PLC']*1.1,
#	parameters[names(parameters)=='PLC'],
#	parameters[names(parameters)=='PLC']*1.2,
#	parameters[names(parameters)=='PLC'],
#	parameters[names(parameters)=='PLC']*1.3,
#	parameters[names(parameters)=='PLC'],
#	parameters[names(parameters)=='PLC']*1.4,
#	parameters[names(parameters)=='PLC'],
...
str(out)
varnames=c('Time (s)','PI','PI(3)P','PI(4)P','PI(5)P','PI(3,5)P2','PI(4,5)P2','PI(3,4)P2','PI(3,4,5)P3',
		'PI3KI_a','PTEN_a','DAG','IP3','PA',
		'ENaC','ASL'
		) 
...
# normalized graphs
 # out[500,]						# getting steady state at time 500
stst = c(	PI 	   = 	290115.435296034,
		PI3P 	   = 	94.6235111384769,
		PI4P 	   = 	11095.7012602329,
		PI5P     = 	97.176844279141, 
		PI35P2   =	21.3415882859489,
...
for (i in 2:ncol(out))
	{
	newc=c(out_n[,i]/as.numeric(stst[i-1]))	# normalizing to steady state got at time 500
...
# tiff("fig_S7a.tiff", height = 20, width = 34, units = 'cm',compression = "lzw", res = 300)
windows()
par(mfrow=c(3,4))
for (i in c(2:9,12,13,14)) 
	{
	plot(out_norm[,1],out_norm[,i],
	type='l',col='orange',
	xlab='Time (min)',ylab='Fold change',main=varnames[i],
	xlim=c(0,120),ylim=c(.85,1),
	lwd=3,cex.axis=1.5,cex.lab=1.5,cex.main=2)
#	text(tmax-2000,out_norm[tmax,i],varnames[i],col='orange')
	}
par(mfrow=c(1,1))
...
### Xu_4_45_data ###
wd = getwd()
setwd(file.path(wd,'7_FigS4_PI45P2_production_acceleration//data_Chang_PIP2_PLC'))
data_Chang = read.csv("data_Chang_PIP2_PLC.csv")
setwd(wd)

# tiff("P2F2_Fig_S4_a.tiff", height = 20, width = 34, units = 'cm',compression = "lzw", res = 300)

par(mar=c(5,6,4,5)+.1)
plot(data_Chang$X,data_Chang$PIP2,type='p',col='black',pch=19,
	ylim=c(0,2),xlim=c(0,80),
	#main='PLC activation',
	xlab='Time (min)',ylab='fold change',
	cex.main=3, cex.lab=3, cex.axis=2
	)
segments(x0=data_Chang$X,y0=data_Chang$LOWER,y1=Xu_data$UPPER,col='black')
polygon (x=c(1,15,15,1),y=c(0,0,2,2),col='pink',border=NA)
points(data_Chang$X,data_Chang$PIP2,type='p',col='black',pch=19,
	ylim=c(0,2),xlim=c(0,80),
	main='PLC activation',
	xlab='Time (min)',ylab='fold change',
	cex.main=3, cex.lab=3, cex.axis=2
...
# Finaly remove the # form the front of all the next code lines
# On the first column put the time of the perturbation
ptime=c(0,10,25,max(t));
# On the second column put the variables to be altered. (ex: 'X')
pvar=c('novar','PLC','PLC','novar')
# On the third the new value for the variable.
pval=c(NA,
	parameters[names(parameters)=='PLC']*25,
	parameters[names(parameters)=='PLC'],
	NA)
perturbations=data.frame(time=ptime,var=pvar,val=pval)
perturbations
...
points((out_norm[421:481,1]-8),out_norm[421:481,7],type='l',col='orange',lwd='5')
points((out_norm[482:1322,1]-9),out_norm[482:1322,7],type='l',col='orange',lwd='5')
points((out_norm[1323:5823,1]-10),out_norm[1323:5823,7],type='l',col='orange',lwd='5')
